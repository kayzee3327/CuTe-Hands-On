...
#define OPT_CALLER call_sgemm_opt86_nt_v5a
...
template <class ProblemShape, class CtaTiler,
          ...
          class TC, class CStride, class CSmemLayout, class TiledMma,
          class Alpha, class Beta>
/// @brief use register prefetch, higher reuse ratio and SMEM buffer
__global__ void sgemm_opt86_nt_v4failed(ProblemShape shape_MNK, CtaTiler cta_tiler,
                                  TA const *A, AStride dA, ASmemLayout sA_layout, TiledCopyA copy_a,
                                  TB const *B, BStride dB, BSmemLayout sB_layout, TiledCopyB copy_b,
                                  TC *C, CStride dC, CSmemLayout, TiledMma mma,
                                  Alpha alpha, Beta beta)
{
  ...
  using SharedStorage = SharedStorageV4failed<TA, TB, ASmemLayout, BSmemLayout>;
...
template <class TA, class TB, class TC,
          class Alpha, class Beta>
void call_sgemm_opt86_nt_v4failed(TA *A, TB *B, TC *C,
                            int M, int N, int K,
                            Alpha alpha, Beta beta)
{
  using namespace cute;
  ...
  int smem_size = int(sizeof(SharedStorageV4failed<TA, TB, decltype(sA), decltype(sB)>));
  sgemm_opt86_nt_v4failed<<<dimGrid, dimBlock, smem_size>>>(prob_shape, cta_tiler,
                                           A, dA, sA, copyA,
                                           B, dB, sB, copyB,
                                           C, dC, sC, mmaC,
                                           alpha, beta);
...
template <class ProblemShape, class CtaTiler,
          ...
          class TC, class CStride, class CSmemLayout, class TiledMma,
          class Alpha, class Beta>
// increase thread tile size compared to v3a
__global__ void sgemm_opt86_nt_v4a(ProblemShape shape_MNK, CtaTiler cta_tiler,
                                  TA const *A, AStride dA, ASmemLayout sA_layout, TiledCopyA copy_a,
                                  TB const *B, BStride dB, BSmemLayout sB_layout, TiledCopyB copy_b,
                                  TC *C, CStride dC, CSmemLayout, TiledMma mma,
                                  Alpha alpha, Beta beta)
{
  ...
  // must use `&` reference, otherwise the whole smem pipeline is copied to local memory
...
template <class TA, class TB, class TC,
          class Alpha, class Beta>
void call_sgemm_opt86_nt_v4a(TA *A, TB *B, TC *C,
                            int M, int N, int K,
                            Alpha alpha, Beta beta)
{
  using namespace cute;
  ...
  dim3 dimGrid(ceil_div(M, bM),
               ceil_div(N, bN));
  int smem_size = (bM + bN) * bK * sizeof(float);
  sgemm_opt86_nt_v4a<<<dimGrid, dimBlock, smem_size>>>(prob_shape, cta_tiler,
                                           A, dA, sA, copyA,
                                           B, dB, sB, copyB,
                                           C, dC, sC, mmaC,
                                           alpha, beta);
...
template <class ProblemShape, class CtaTiler,
          ...
          class TC, class CStride, class CSmemLayout, class TiledMma,
          class Alpha, class Beta>
// use SMEM double buffer compared to v4a
__global__ void sgemm_opt86_nt_v4b(ProblemShape shape_MNK, CtaTiler cta_tiler,
                                  TA const *A, AStride dA, ASmemLayout sA_layout, TiledCopyA copy_a,
                                  TB const *B, BStride dB, BSmemLayout sB_layout, TiledCopyB copy_b,
                                  TC *C, CStride dC, CSmemLayout, TiledMma mma,
                                  Alpha alpha, Beta beta)
{
  ...
  // SMEM buffer prologue
  int K_PIPE_MAX = size<3>(tAsA);
  int k_tile_count = K_TILE_MAX;
  int k_tile_next = 0;
  copy(copy_a, tAgA(_,_,_,k_tile_next), tArA);
  copy(copy_b, tBgB(_,_,_,k_tile_next), tBrB);
  k_tile_count--;
  ...
  {
    copy(tArA, tAsA(_,_,_,k_pipe));
    copy(tBrB, tBsB(_,_,_,k_pipe));
    __syncthreads();
    copy(copy_a, tAgA(_,_,_,k_tile_next), tArA);
    copy(copy_b, tBgB(_,_,_,k_tile_next), tBrB);
    k_tile_count--;
  ...
  while (k_tile_count + K_PIPE_MAX > 0)
  {
    CUTE_UNROLL
    for (int k_block = 0; k_block < K_BLOCK_MAX; k_block++)
    {
      copy(s2r_tiled_copy_a, tXsA_p(_,_,k_block), tXrA(_,_,k_block));
      copy(s2r_tiled_copy_b, tXsB_p(_,_,k_block), tXrB(_,_,k_block));
...
template <class TA, class TB, class TC,
          class Alpha, class Beta>
void call_sgemm_opt86_nt_v4b(TA *A, TB *B, TC *C,
                            int M, int N, int K,
                            Alpha alpha, Beta beta)
{
  using namespace cute;
  ...
  // note that smem_size must be in unit of bytes
  sgemm_opt86_nt_v4b<<<dimGrid, dimBlock, smem_size>>>(prob_shape, cta_tiler,
                                           A, dA, sA, copyA,
                                           B, dB, sB, copyB,
                                           C, dC, sC, mmaC,
                                           alpha, beta);
...
template <class ProblemShape, class CtaTiler,
          ...
          class TC, class CStride, class CSmemLayout, class TiledMma,
          class Alpha, class Beta>
// use register prefetch compared to v4b
__global__ void sgemm_opt86_nt_v4c(ProblemShape shape_MNK, CtaTiler cta_tiler,
                                  TA const *A, AStride dA, ASmemLayout sA_layout, TiledCopyA copy_a,
                                  TB const *B, BStride dB, BSmemLayout sB_layout, TiledCopyB copy_b,
                                  TC *C, CStride dC, CSmemLayout, TiledMma mma,
                                  Alpha alpha, Beta beta)
...
template <class TA, class TB, class TC,
          class Alpha, class Beta>
void call_sgemm_opt86_nt_v4c(TA *A, TB *B, TC *C,
                            int M, int N, int K,
                            Alpha alpha, Beta beta)
{
  using namespace cute;
  ...
  int smem_size = int(sizeof(SharedStorage<TA, TB, decltype(sA), decltype(sB)>));
  // note that smem_size must be in unit of bytes
  sgemm_opt86_nt_v4c<<<dimGrid, dimBlock, smem_size>>>(prob_shape, cta_tiler,
                                           A, dA, sA, copyA,
                                           B, dB, sB, copyB,
                                           C, dC, sC, mmaC,
                                           alpha, beta);
...
template <class ProblemShape, class CtaTiler,
          ...
          class TC, class CStride, class CSmemLayout, class TiledMma,
          class Alpha, class Beta>
// fix epilogue compared to v4c
__global__ void sgemm_opt86_nt_v4d(ProblemShape shape_MNK, CtaTiler cta_tiler,
                                  TA const *A, AStride dA, ASmemLayout sA_layout, TiledCopyA copy_a,
                                  TB const *B, BStride dB, BSmemLayout sB_layout, TiledCopyB copy_b,
                                  TC *C, CStride dC, CSmemLayout, TiledMma mma,
                                  Alpha alpha, Beta beta)
{
  ...
  // C is n-major while A/B is m-major
  //  so we need a transpose in the epilogue copy
  auto copy_epi =
  make_tiled_copy(Copy_Atom<UniversalCopy<uint128_t>, TC>{},
                  Layout<Shape<_16, _16>, Stride<_1, _16>>{},
                  Layout<Shape<_4, _4>, Stride<_4, _1>>{});
  ThrCopy thr_copy_epi = copy_epi.get_slice(threadIdx.x);
  ...
  if(thread0()) {
    print(copy_epi); print("\n");
    print(thr_copy_epi); print("\n");
    print(tXrD); print("\n");
    print(tXgD); print("\n");
  }
  ...
  CUTE_UNROLL
    ...
    // First get old C into a register fragment.
    // This load may still be scalar initially; vectorize it separately later.
    // We may further optimize it with SMEM.
    Tensor tCrCold = make_fragment_like(tCrC);
    Tensor tXgCold = thr_copy_epi.partition_S(gC);
    Tensor tXrCold = thr_copy_epi.retile_D(tCrCold);
    copy(copy_epi, tXgCold, tXrCold);

    CUTE_UNROLL
    for (int i = 0; i < size(tCrD); ++i) {
      tCrD(i) = alpha * tCrC(i) + beta * tCrCold(i);
    }
  }

  copy(copy_epi, tXrD, tXgD);
#endif
}

template <class TA, class TB, class TC,
          class Alpha, class Beta>
void call_sgemm_opt86_nt_v4d(TA *A, TB *B, TC *C,
                            int M, int N, int K,
                            Alpha alpha, Beta beta)
{
  using namespace cute;
  ...
  int smem_size = int(sizeof(SharedStorage<TA, TB, decltype(sA), decltype(sB)>));
  // note that smem_size must be in unit of bytes
  sgemm_opt86_nt_v4d<<<dimGrid, dimBlock, smem_size>>>(prob_shape, cta_tiler,
                                           A, dA, sA, copyA,
                                           B, dB, sB, copyB,
                                           C, dC, sC, mmaC,
                                           alpha, beta);
...
template <class ProblemShape, class CtaTiler,
          ...
          class TC, class CStride, class CSmemLayout, class TiledMma,
          class Alpha, class Beta>
// improve register prefetch scheduling compared to v4d
__global__ void sgemm_opt86_nt_v4e(ProblemShape shape_MNK, CtaTiler cta_tiler,
                                  TA const *A, AStride dA, ASmemLayout sA_layout, TiledCopyA copy_a,
                                  TB const *B, BStride dB, BSmemLayout sB_layout, TiledCopyB copy_b,
                                  TC *C, CStride dC, CSmemLayout, TiledMma mma,
                                  Alpha alpha, Beta beta)
...
template <class TA, class TB, class TC,
          class Alpha, class Beta>
void call_sgemm_opt86_nt_v4e(TA *A, TB *B, TC *C,
                            int M, int N, int K,
                            Alpha alpha, Beta beta)
{
  using namespace cute;
  ...
  int smem_size = int(sizeof(SharedStorage<TA, TB, decltype(sA), decltype(sB)>));
  // note that smem_size must be in unit of bytes
  sgemm_opt86_nt_v4e<<<dimGrid, dimBlock, smem_size>>>(prob_shape, cta_tiler,
                                           A, dA, sA, copyA,
                                           B, dB, sB, copyB,
                                           C, dC, sC, mmaC,
                                           alpha, beta);
...
template <class ProblemShape, class CtaTiler,
          ...
          class TC, class CStride, class CSmemLayout, class TiledMma,
          class Alpha, class Beta>
// introduce cp.async compared to v4d
__global__ void sgemm_opt86_nt_v5(ProblemShape shape_MNK, CtaTiler cta_tiler,
                                  TA const *A, AStride dA, ASmemLayout sA_layout, TiledCopyA copy_a,
                                  TB const *B, BStride dB, BSmemLayout sB_layout, TiledCopyB copy_b,
                                  TC *C, CStride dC, CSmemLayout, TiledMma mma,
                                  Alpha alpha, Beta beta)
{
  ...
  while (k_tile_count + K_PIPE_MAX - 1 > 0)
  {
    CUTE_UNROLL
    for (int k_block = 0; k_block < K_BLOCK_MAX; k_block++)
    {
...
template <class TA, class TB, class TC,
          class Alpha, class Beta>
void call_sgemm_opt86_nt_v5(TA *A, TB *B, TC *C,
                            int M, int N, int K,
                            Alpha alpha, Beta beta)
{
  using namespace cute;
  ...
  using atom_g2s = Copy_Atom<
    SM80_CP_ASYNC_CACHEGLOBAL<uint128_t>,
    float
  >;

  TiledCopy copyA = make_tiled_copy(atom_g2s{},
                                    Layout<Shape<_32, _8>>{},
                                    Layout<Shape<_4, _1>>{});
  TiledCopy copyB = make_tiled_copy(atom_g2s{},
                                    Layout<Shape<_32, _8>>{},
                                    Layout<Shape<_4, _1>>{});
  ...
  int smem_size = int(sizeof(SharedStorage<TA, TB, decltype(sA), decltype(sB)>));
  // note that smem_size must be in unit of bytes
  sgemm_opt86_nt_v5<<<dimGrid, dimBlock, smem_size>>>(prob_shape, cta_tiler,
                                           A, dA, sA, copyA,
                                           B, dB, sB, copyB,
                                           C, dC, sC, mmaC,
                                           alpha, beta);
                                  ...
                                  Alpha alpha, Beta beta)
{
  ...
  make_tiled_copy(Copy_Atom<UniversalCopy<uint128_t>, TC>{},
                  ...
                  Layout<Shape<_4, _4>, Stride<_4, _1>>{});
  ThrCopy thr_copy_epi = copy_epi.get_slice(threadIdx.x);
  Tensor tXrC = thr_copy_epi.retile_S(tCrC);
  Tensor tXgC = thr_copy_epi.partition_D(gC);
  ...
  copy(copy_epi, tXrC, tXgC);
}

template <class TA, class TB, class TC,
          class Alpha, class Beta>
void call_sgemm_opt86_nt_v5a(TA *A, TB *B, TC *C,
                            int M, int N, int K,
                            Alpha alpha, Beta beta)
{
  using namespace cute;
  ...
  int smem_size = int(sizeof(SharedStorage<TA, TB, decltype(sA), decltype(sB)>));
  // note that smem_size must be in unit of bytes
  sgemm_opt86_nt_v5a<<<dimGrid, dimBlock, smem_size>>>(prob_shape, cta_tiler,
                                           A, dA, sA, copyA,
                                           B, dB, sB, copyB,
                                           C, dC, sC, mmaC,
                                           alpha, beta);
